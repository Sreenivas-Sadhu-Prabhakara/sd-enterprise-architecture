...
| **Asset Type** | Architecture Blueprint |
| **Control Record** | Architecture Blueprint Design |
...
| POST | `/v1/architecture-blueprint-design/initiate` | Initiate |
| GET | `/v1/architecture-blueprint-design` | Retrieve (list) |
| GET | `/v1/architecture-blueprint-design/{crId}/retrieve` | Retrieve |
| PUT | `/v1/architecture-blueprint-design/{crId}/update` | Update |
| PUT | `/v1/architecture-blueprint-design/{crId}/control` | Control — body `{"action": "suspend"\|"resume"\|"terminate"}` |
...
curl -X POST localhost:8080/v1/architecture-blueprint-design/initiate -H 'content-type: application/json' -d '{"note":"hello"}'
```
...
```bash
mvn -B verify
docker build -t bian/sd-enterprise-architecture:0.1.0 .
```

## Deploy (Helm → K8s with Cilium mesh)

```bash
helm upgrade --install sd-enterprise-architecture ./helm -n bian-business-support
```

Exposed through the platform Gateway at path prefix `/sd-enterprise-architecture` (Cilium Gateway API). Mesh policy (`CiliumNetworkPolicy`) allows: gateway ingress, same-area peers, Prometheus — everything else denied.
